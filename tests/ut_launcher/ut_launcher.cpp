...
void Ut_Launcher::testUpdatingButtonPlacementsOnPage()
{
    ...
    fileExists = false; // installer-extra entries don't exist
...
void Ut_Launcher::testUpdateButtonState()
{
    ...
    launcher->updateButtonState(desktopEntry, packageName, ApplicationPackageMonitor::Downloading, true);
    QCOMPARE(launcher->model()->launcherPages().count(), 1);
    ...
    launcher->updateButtonState(desktopEntry, packageName, ApplicationPackageMonitor::Installing, true);
    QCOMPARE(launcher->model()->launcherPages().count(), 1);
    QCOMPARE(launcher->model()->launcherPages().at(0)->model()->launcherButtons().count(), 1);
...
void Ut_Launcher::testUpdateButtonStateForButtonInLauncher()
{
    ...
    gLauncherButtonStub->stubSetReturnValue("desktopEntry", desktopEntry->fileName());
    launcher->updateButtonState(desktopEntry, QString(), ApplicationPackageMonitor::Installing, true);
    QCOMPARE(launcher->model()->launcherPages().count(), 1);
    QCOMPARE(launcher->model()->launcherPages().at(0)->model()->launcherButtons().count(), 1);
    QCOMPARE(gLauncherButtonStub->stubCallCount("setState"), 1);
    QCOMPARE(gLauncherButtonStub->stubLastCallTo("setState").parameter<LauncherButtonModel::State>(0), LauncherButtonModel::Installing);
...
void Ut_Launcher::testSetOperationErrorWhenButtonFoundFromLauncher()
{
    ...
    gApplicationPackageMonitorStub->stubSetReturnValue("isInstallerExtraEntry", true);
    launcher->updateButtonState(installerExtraEntry, QString(), ApplicationPackageMonitor::Broken, true);
...
void Ut_Launcher::testButtonInfoIsUpdatedFromDesktopEntryWhenButtonStateIsUpdatedFromApplicationsEntry()
{
    ...
    launcher->updateButtonState(installedApplicationsEntry, "package", ApplicationPackageMonitor::Installed, true);
...
void Ut_Launcher::testSetOperationErrorWhenButtonHasPlaceholder()
{
    ...
    launcher->updateButtonState(entry, QString(), ApplicationPackageMonitor::Broken, true);

    QCOMPARE(gLauncherButtonStub->stubLastCallTo("setState").parameter<LauncherButtonModel::State>(0), LauncherButtonModel::Broken);
}

void Ut_Launcher::testConvertingToPlaceholderWhenButtonIsRemovedAndInstallerExtraRemains()
{
    QSharedPointer<MDesktopEntry> installExtraEntry(new MDesktopEntry("installer-extra/testRemoveInstallExtra.desktop"));
    ...
    gApplicationPackageMonitorStub->stubSetReturnValue("isInstallerExtraEntry", false);
    gApplicationPackageMonitorStub->stubSetReturnValue("toInstallerExtraEntryPath", installExtraEntry->fileName());
    gLauncherButtonStub->stubSetReturnValue("desktopEntry", applicationsEntry->fileName());
    launcher->removeLauncherButton(applicationsEntry->fileName());

    // Nothing removed, since installer-extra entry still exists
    QCOMPARE(launcher->model()->launcherPages().at(0)->model()->launcherButtons().count(), BUTTONS_PER_PAGE);
    QCOMPARE(gLauncherButtonStub->stubCallCount("updateFromDesktopEntry"), 1);
    QCOMPARE(gLauncherButtonStub->stubLastCallTo("updateFromDesktopEntry").parameter<const QSharedPointer<MDesktopEntry> >(0)->fileName(), installExtraEntry->fileName());
}
...
void Ut_Launcher::testConnectionsAfterLauncherInitialization()
{
    ...
    QVERIFY(disconnect(packageMonitor, SIGNAL(packageStateUpdated(QSharedPointer<MDesktopEntry>, QString, ApplicationPackageMonitor::PackageState, bool)),
            launcher, SLOT(updateButtonState(QSharedPointer<MDesktopEntry>, QString, ApplicationPackageMonitor::PackageState, bool))));
...
void Ut_Launcher::testThatLauncherIconIsRemovedWhenApplicationUninstallProgressIsStarted()
{
    ...
    launcher->updateButtonState(uninstallableInstallerExtraEntry, "testApp", ApplicationPackageMonitor::Uninstalling, true);
...
void Ut_Launcher::testPlaceholderButtonIsNotAddedIfApplicationDesktopEntryIsInvalid()
{
    ...
    launcher->updateButtonState(desktopEntry, "pkg", ApplicationPackageMonitor::Installed, true);
    QCOMPARE(launcher->model()->launcherPages().count(), 0);
}
